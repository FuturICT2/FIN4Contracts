...
module.exports = async function (deployer) {
  ...
  await Promise.all(
    verifierTypeContracts.map((contract) => deployer.deploy(contract))
  );
  const verifierTypeInstances = await Promise.all(
    verifierTypeContracts.map((contract) => contract.deployed())
  );
  await Promise.all(
    verifierTypeInstances.map(({ address }) =>
      Fin4VerifyingInstance.addVerifierType(address)
    )
  );
  ...
  await verifierTypeInstances[0].setFin4GroupsAddress(
    Fin4GroupsInstance.address
  );
  await verifierTypeInstances[0].setFin4MessagingAddress(
    Fin4MessagingInstance.address
  );
  // SelfieTogether
  await verifierTypeInstances[1].setFin4GroupsAddress(
    Fin4GroupsInstance.address
  );
  await verifierTypeInstances[1].setFin4MessagingAddress(
    Fin4MessagingInstance.address
  );
  // Blacklisting
  await verifierTypeInstances[2].setFin4GroupsAddress(
    Fin4GroupsInstance.address
  );
  // Whitelisting
  await verifierTypeInstances[3].setFin4GroupsAddress(
    Fin4GroupsInstance.address
  );
  // SpecificAddress
  await verifierTypeInstances[5].setFin4MessagingAddress(
    Fin4MessagingInstance.address
  );
  // TokenCreatorApproval
  await verifierTypeInstances[6].setFin4MessagingAddress(
    Fin4MessagingInstance.address
  );
  // Picture
  await verifierTypeInstances[8].setFin4MessagingAddress(
    Fin4MessagingInstance.address
  );
  ...
  await Fin4CollectionsInstance.setFin4GroupsAddress(
    Fin4GroupsInstance.address
  );
  ...
  let data =
    ...
    "';\n" +
    "const networkNews = '" +
    verifierTypeInstances +
    "';\n" +
    "export { Fin4MainAddress, networkName };\n";
  fs.writeFile(
    path.join(
      __dirname,
      config.DEPLOYMENT_INFO_SAVING_LOCATION + "/deployment-info.js"
    ),
    data,
    (err) => {
